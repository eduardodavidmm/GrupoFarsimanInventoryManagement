...
<?php
  if(isset($_POST['add'])){
   ...
   if(find_by_groupName($_POST['group-name']) === false ){
     $session->msg('d','<b>Sorry!</b>Ingresaste un grupo existente');
     redirect('add_group.php', false);
   }elseif(find_by_groupLevel($_POST['group-level']) === false) {
     $session->msg('d','<b>Sorry!</b>Ingresaste un Rol Existente');
     redirect('add_group.php', false);
   }
   if(empty($errors)){
        ...
        if($db->query($query)){
          //sucess
          $session->msg('s',"Creaste un Grupo");
          redirect('add_group.php', false);
        } else {
          //failed
          $session->msg('d','Error al crear grupo');
          redirect('add_group.php', false);
        }
   } else {
...
<div class="login-page">
    <div class="text-center">
       <h3>Añadir Grupo de Usuarios</h3>
     </div>
     <?php echo display_msg($msg); ?>
      <form method="post" action="add_group.php" class="clearfix">
        ...
        <div class="form-group">
              <label for="level" class="control-label">Rol</label>
